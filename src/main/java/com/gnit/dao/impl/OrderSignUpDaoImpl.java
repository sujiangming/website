...
@Repository
public class OrderSignUpDaoImpl extends BaseDaoImpl {
	
	/**
	 * 分页查询免费试听列表
	 */
	public PageList<OrderSignUp> selectList(Map<String, Object> parameter) {
		String hql = "from " + OrderSignUp.class.getName();
	 ...
	 * @param freeLearn
	 */
	public void saveOrUpdate(OrderSignUp freeLearn) {
	 ...
	 * @return
	 */
	public OrderSignUp getObjectByPhoneNum(String phoneNumber) {
		return this.findObject("from " + OrderSignUp.class.getName() + " where phoneNumber='" + phoneNumber + "'");
	}
	 ...
	 * @return
	 */
	public OrderSignUp getRoleById(String id) {
		return this.findObject("from "+OrderSignUp.class.getName()+" where id='" + id + "'");
	}
	...
	public void delete(String id) {
		...
		for (int i = 0; i < roleIds.length; i++) {
			OrderSignUp role = this.getRoleById(roleIds[i]);
			if (role != null) {
				session.delete(role);
			}
		}
