...
var TableInit = function() {
	...
	function operateFormatter(value, row, index) {
		var html = '<a id="a_show">查看 <span style="color:#CCC">|</span></a><a id="a_edit">编辑<span style="color:#CCC">|</span> </a><a id="a_reset">重置 </a>';
		return html;
	}
	// 操作列的事件
	window.operateEvents = {
		...
		'click #a_reset' : function(e, value, row, index) {
			resetPwd(row);
		}
	};
...
function resetPwd(obj){
	layer.confirm("您确定要重置该用户密码吗?", {
		skin : 'layui-layer-molv',
		btn : [ '确定', '取消' ]
	}, function() {
		var url = getRootPath() + "user/reset";
		$.post(url, {
			id : obj.id
		}, function(data) {
			if (data.status == "1") {
