...
function initBtnEvent() {
	...
	$("#btn_del").bind('click', function() {
		...
		layer.confirm("您确定要删除所选信息吗?", {
			...
			var url = getRootPath() + "orderSignUp/delete";
			$.post(url, {
				id : selections + ""
			}, function(data) {
				if (data.status == "1") {
...
var TableInit = function() {
	...
	oTableInit.Init = function() {
		$('#roleInfo').bootstrapTable({
			url : getRootPath() + "orderSignUp/list", // 请求后台的URL（*）
			...
			columns : [ {
				...
				formatter: function (value, row, index){ // 单元格格式化函数
              		  var div = "<div style='width:50px;word-break: break-all;'>"+value+"</div>";//调列宽，在td中嵌套一个div，调整div大小
                  	  return div;
                }
			}, {
				field : 'ip',
...
function openSaveButton() {
	...
	$.ajax({
		type : "post",
		url : getRootPath() + "orderSignUp/saveOrUpdate",
		data : addJson,
		dataType : "json",
		async : true,
