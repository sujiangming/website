...
function initBtnEvent() {
	...
	$("#btn_del").bind('click', function() {
		...
		layer.confirm("您确定要删除所选信息吗?", {
			...
			var url = getRootPath() + "preschoolTest/delete";
			$.post(url, {
				id : selections + ""
			}, function(data) {
				if (data.status == "1") {
...
var TableInit = function() {
	...
	oTableInit.Init = function() {
		$('#roleInfo').bootstrapTable({
			url : getRootPath() + "preschoolTest/list", // 请求后台的URL（*）
...
function openSaveButton() {
	...
	$.ajax({
		type : "post",
		url : getRootPath() + "preschoolTest/saveOrUpdate",
		data : addJson,
		dataType : "json",
		async : true,
