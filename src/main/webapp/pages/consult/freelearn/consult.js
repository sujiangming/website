...
function initBtnEvent() {
	...
	$("#btn_del").bind('click', function() {
		...
		layer.confirm("您确定要删除所选信息吗?", {
			...
			var url = getRootPath() + "freeLearn/delete";
			$.post(url, {
				id : selections + ""
			}, function(data) {
				if (data.status == "1") {
...
var TableInit = function() {
	...
	oTableInit.Init = function() {
		$('#roleInfo')
				.bootstrapTable(
						{
							url : getRootPath() + "freeLearn/list", // 请求后台的URL（*）
							...
							columns : [
									...
									},
									{
										...
										formatter : function(value, row, index) { // 单元格格式化函数
											var div = "<div style='width:50px;word-break: break-all;'>"
	...
	function operateFormatter(value, row, index) {
		var html = '<div style="width:80px;align:center;"><a id="a_check">查看 <span style="color:#CCC">|</span> </a>'
				+ '<a id="a_edit">处理</a></div>'
		return html;
	}
...
function openSaveButton() {
	...
	$.ajax({
		type : "post",
		url : getRootPath() + "freeLearn/saveOrUpdate",
		data : addJson,
		dataType : "json",
		async : true,
